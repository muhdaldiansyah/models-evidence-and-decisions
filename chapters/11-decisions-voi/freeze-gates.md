...
| | |
|---|---|
| Chapter | **Decisions Under Uncertainty and Value of Information** |
| Manuscript | complete — 7 sections, 10,664 words |
| Budget | 33 pages / 7 learning hours |
...
| Terminology block | provisional, pending [Decision 0018](../../decisions/0018-chapter11-decision-terminology-and-boundary.md) |
...
**The book's longest thread closes here, on a negative result.** The monetisation of household-events carries more of the result than any other number in the chapter, so Gate 1 is unusually load-bearing. The pilot question is whether a reader accepts a negative result as a result, or reads it as the chapter having failed.
...
6. **Chapter audit.** The 33-page / 7-hour architecture holds unless evidence justifies reopening it; all 7 sections still serve their governed jobs; central question and core competence intact; reveal order coherent; citation keys resolve and source-note cautions are respected; no exercise exposes an answer, rubric, or parallel form before production.
7. **Freeze decision.** `FREEZE` · `FREEZE WITH DEFERRED NON-BLOCKING ITEMS` · `REVISE AND RE-PILOT` · `REOPEN GOVERNED DECISION`.

**Gate 7 additionally requires that [Decision 0018](../../decisions/0018-chapter11-decision-terminology-and-boundary.md) has been adjudicated.** This chapter's terminology and scope are provisional until it is, and a chapter cannot freeze on a provisional boundary.
