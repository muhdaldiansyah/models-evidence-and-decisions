...
For example, a calculation may support the claim that higher production increases projected storage. It cannot, by itself, determine whether the additional operating cost is worth paying, how precautionary the utility should be, or what public burden is acceptable. Those judgments require values, obligations, requirements, constraints, or other evaluative premises.
...
Once intended use, target, decision-maker, and alternatives are visible, another source of error appears: two questions can mention the same system and outcome while asking for different kinds of answers.
...
The same records may be relevant to all four. They do not support the same claim automatically.

A useful first pass therefore screens the question in layers. The layers are neither an exhaustive taxonomy nor mutually exclusive boxes. Their job is to expose what kind of evidence and reasoning the answer will require.

### First layer: what would happen, and what should matter?

Begin before technical labels. Ask whether a component concerns:
...
Positive does not mean “certain,” “objective,” or “merely descriptive.” A forecast is positive when it asks what will happen. An intervention or counterfactual question can also be positive when it asks what would happen under a specified change.

Normative does not mean “unsupported opinion.” Practical judgments about what should count as preferable may be constrained by law, professional obligation, policy, ethical reasoning, institutional purpose, or stated values. Structured decision work likewise separates facts and consequence estimates from values and objectives used to evaluate them [@bradley2016structured].
...
A consequential problem often contains both layers. The point is to notice where evidence about what would happen ends and evaluation of those consequences begins.
...
Association alone is not enough to establish that intervention effect; causal conclusions require additional assumptions or design information [@pearl2009causal]. Formal causal identification belongs to Chapter 7.
...
This is not just any hypothetical story. It compares the factual situation with an alternative course under a specified change.

These forms can overlap. A decision analysis may require a demand prediction, an intervention estimate for a conservation action, and a counterfactual assessment of an earlier missed action. The mistake is to force the whole problem into one label or to assume that evidence adequate for one claim automatically supports the others.
...
A problem is **dynamic** when relevant states or conditions evolve through time. Dynamics do not require strategic behavior. A simple pendulum changes through time, yet it does not adapt because it anticipates our model.

At the introductory depth used here, **feedback** occurs when effects of a process or action return through the system and influence later behavior, outcomes, information, or actions. Chapter 1 does not classify feedback as positive or negative or analyze stability; those topics belong later.

Repeated choice is separate again. A decision can be revisited as state or information changes even when there is no strategic opponent.

People and institutions may also **adapt** or respond **strategically**. Adaptation is broader: behavior changes after conditions, information, experience, or intervention change. Strategic response is narrower: behavior changes partly because a rule, incentive, metric, prediction, policy, or anticipated action of others makes some response more advantageous. The presence of people does not automatically make a problem strategic.

### Two short contrasts

A pendulum used to estimate local gravitational acceleration, predict short-horizon angular position, or design a durable clock is the same physical system under different intended uses. The representation and adequacy requirements can change, but the example remains dynamic without being adaptive or strategic.

Now consider a 20-item student assessment used to rank applicants, diagnose prerequisite skills, or choose the next instructional activity. The observed responses are the same kind of record, but the intended use and target differ. If the score becomes consequential, students or institutions may also change behavior in response to the rule.
...
A strong answer distinguishes prediction from intervention. The first asks whether the variable helps forecast an outcome; the second asks what would happen if an action changed something in the system.
...
The purpose is not to reward labeling. It is to make the next question sharper:
...
The important feature of 4.7 ML is not its numerical precision. It is its **decision sensitivity**. The projection sits only 0.2 ML above the case reserve. A modest error in starting storage or demand could reverse the reserve-crossing conclusion. That tells the analyst what to ask next: which uncertain input can be checked soon enough, and accurately enough, to change the immediate action?

The calculation has not established that 10.8 ML is the physical storage level. It has not established how demand would change after a conservation action. It has not chosen among actions. The next step should therefore be driven by decision relevance, not by a reflex to make the forecast more elaborate.
...
Why is that information worth considering? Not because “more data” are automatically better. The check has three properties that make it decision-relevant here:
...
Information that arrives after the decision is irreversible, or that cannot move the action, would be much less useful for this immediate purpose. Chapter 11 will later formalize questions about the value of information; here the qualitative screen is enough.
...
The target did not change. The seven-day demand forecast did not change. The reserve did not change. What changed was our evidence about the current physical state, and that change was sufficient to alter the practical conclusion.

**Revision 1: measurement / observation revision.**

We move backward because the dashboard record was not the target itself. The target was physical usable storage. The record was evidence about that target, produced by an observation process. Once that process failed in a decision-relevant way, the current-state belief—and therefore the urgency of the decision—had to change.
...
Now consider the demand evidence. Historical hot-weather demand records can be useful for prediction. But predictive usefulness does not automatically tell us what would happen if we intervene. An observed association alone does not establish the effect of an action [@shmueli2010predict; @pearl2009causal].
...
So the evidence is uneven. We now have strong case-specific reason to revise current storage. We have a forecast for demand under a no-new-action condition. We have weaker evidence about the causal effect of a new conservation request. A first pass should expose that asymmetry rather than compress it into one confidence statement.

Before choosing, ask what remains uncertain **and which uncertainties matter to the next action**. We still do not know the exact conservation effect, future forecast error, or whether the higher-production action will be sufficient under a hotter-than-expected demand path. Those are reasons to compare alternatives and preserve monitoring, not reasons to pretend no decision can be made yet.
...
| Increase production | Director; effect after 6 h | Adds storage margin at $2,000 per 24 h |
...
Higher production helps, but it does not make the conclusion insensitive to demand assumptions. The central calculation and the stress calculation support different statements: one says the action creates margin under the central forecast; the other says that margin is not robust to the supplied high-demand stress.
...
Make the evaluative bridge explicit. For this worked recommendation, suppose the decision-maker accepts these case premises:
...
These premises are not empirical findings hidden in the data. They state how consequences are being judged for this decision. A different legitimate set of premises could support a different recommendation.
...
The staged form also matters. It avoids treating the seven-day horizon as one irreversible commitment. The director can act on what is sufficiently supported now while preserving the ability to revise when better evidence arrives.
...
Predictions used to support decisions can influence the outcomes they are intended to predict when people respond to them [@perdomo2020performative]. Chapter 1 uses a broader version of that warning as a pedagogical synthesis: a deployed rule, metric, model, or policy can also become part of a responsive environment. More generally, feedback, delays, omitted responses, and narrow boundaries can make learning from interventions difficult [@sterman2006evidence].
...
What did not change is equally important. The intended use is still to support the utility's short-horizon operating decision, the physical reserve remains a case constraint, and the director's authority has not changed. Revision should be targeted to the part of the formulation that failed, not used as an excuse to rebuild everything.
...
The first pass has not completed a professional water-utility analysis. It has done something more basic and more transferable: it has exposed what must be learned next. The observation problem routes toward Chapters 3–4; forecast uncertainty and intervention evidence toward Chapters 6–8; values and choice toward Chapters 10–11; and repeated, responsive behavior toward Chapters 13–15. The first pass is successful partly because it tells us where specialist machinery is now warranted.
...
A new weather forecast or newer trusted storage reading may change numbers while leaving the target, comparison, boundary, authority, alternatives, and intended use intact. Other discoveries are different. If the dashboard record is not an adequate representation of physical storage, the observation problem must be reconsidered. If conservation changes the process generating future demand, a no-new-action forecast is no longer the right forecast. If the proposed action belongs to another authority, the decision situation itself has been misstated.
...
The size of the numerical change does not decide which column applies. A small discrepancy can force major reframing if it exposes the wrong target or observation process; a large update can remain ordinary adjustment if the formulation still fits the use.
...
Neither is absurd. If the dashboard is well verified and forecast uncertainty dominates, Formulation A may deserve priority. If less than 1 ML of starting-state error can change the action and an independent check arrives in 25 minutes, Formulation B becomes more important.

The point is not to generate rivals for sport. It is to keep a plausible alternative formulation available when the current framing depends heavily on an assumption that could fail.
...
Return briefly to the pendulum. The same physical object can be used to estimate local gravity, predict short-horizon position, or regulate a durable clock. Intended use changes what the representation must preserve and what counts as adequate. The object did not change; the question did.

That is why model criticism should not demand that every representation contain every feature of reality. The better question is whether an omitted feature could matter for the stated use. Chapters 2 and 5 develop that judgment more carefully.

### A faded case: one test, two uses

A school has a **20-item mathematics assessment** and one total score for each student. Administrators propose using it to:

1. rank applicants for a limited advanced program;
2. diagnose which prerequisite skills current students need to review.

No psychometrics knowledge is required. Before reading further, write a short response:
...
A defensible comparison notices that the same **record** can support different inquiries. Ranking may concern relative standing under a selection purpose; diagnosis concerns which skills require attention. A total score may be relevant to both without carrying the same information for both. Evidence that students with the same total score miss very different items would be enough to challenge a formulation that treats the total as an adequate answer to both questions.

If the score becomes consequential, another revision may eventually be needed because students or institutions may respond to the rule itself. Chapter 1 only screens for that possibility.

### Different failures require different repairs

The examples suggest several reasons to go backward: intended-use failure, target failure, observation failure, evidence failure, alternative or authority failure, evaluation failure, and response failure. These labels are reminders, not a checklist.

If the record is wrong, collecting more of the same forecast data may not help. If an intervention effect is not established, polishing predictive accuracy may not answer the causal question. If authority is misstated, a more precise calculation does not make an infeasible action feasible. If action changes behavior, a model fitted to the pre-action environment may need more than a parameter update.

### Know when to hand the problem forward

A strong first pass often ends by identifying the kind of expertise needed next. Common routes can be grouped:
...
The goal is not to prove that four hours of instruction have produced durable expertise. It is to obtain initial evidence about whether you can reconstruct the reasoning without being led through every step.
...
Identify at least **six consequential failures**. For each one, write:
...
**Stop here until you have completed the diagnosis.** Then open [`diagnosis-feedback.md`](diagnosis-feedback.md) and compare your reasoning before continuing.
...
For self-study, choose the domain that is **less familiar** from the titles alone. Open only that form. **Do not preview the other form.** It is reserved for the delayed retest.

Do not consult the Chapter 1 map, worked water solution, or scoring rubric until you have finished a one-page first-pass analysis. The current pilot target is **30–40 minutes**; that is an operational design target, not a universal standard.

State only assumptions that are needed. Do not invent missing domain facts; if an important fact is absent, identify it and explain whether learning it could change the decision. The selected form supplies the domain facts needed for the reasoning task.

**Finish the one-page analysis before opening the rubric.** Then use [`transfer-rubric.md`](transfer-rubric.md) for dimension-level feedback and the post-task self-explanation prompt.

### Return to your five-minute opening attempt

Retrieve the response you wrote at the beginning of the chapter. Do not ask whether the later answer simply contains more terminology. Ask whether the structure changed.
...
Close or cover the chapter map. From memory, reconstruct the five-part navigation used in the worked case and add arrows showing at least two ways later evidence can send reasoning backward.
