...
| | |
|---|---|
| Chapter | **Dynamics, Feedback, and Stability** |
| Manuscript | complete — 8 sections, 9,927 words |
| Budget | 28 pages / 5 learning hours |
...
| Terminology block | provisional, pending [Decision 0020](../../decisions/0020-chapter13-dynamics-terminology-and-boundary.md) |
...
**The first chapter run forward in time**, and the first whose case is a simulation rather than a snapshot. Every figure was computed rather than asserted, so a realism failure at Gate 1 would invalidate arithmetic rather than prose. The four-day loop delay carries the entire argument and is the first thing a reviewer should be pointed at.
...
6. **Chapter audit.** The 28-page / 5-hour architecture holds unless evidence justifies reopening it; all 8 sections still serve their governed jobs; central question and core competence intact; reveal order coherent; citation keys resolve and source-note cautions are respected; no exercise exposes an answer, rubric, or parallel form before production.
7. **Freeze decision.** `FREEZE` · `FREEZE WITH DEFERRED NON-BLOCKING ITEMS` · `REVISE AND RE-PILOT` · `REOPEN GOVERNED DECISION`.

**Gate 7 additionally requires that [Decision 0020](../../decisions/0020-chapter13-dynamics-terminology-and-boundary.md) has been adjudicated.** This chapter's terminology and scope are provisional until it is, and a chapter cannot freeze on a provisional boundary.
