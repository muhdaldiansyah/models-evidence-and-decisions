...
| | |
|---|---|
| Chapter | **Optimization, Robustness, and Adaptive Plans** |
| Manuscript | complete — 8 sections, 12,325 words |
| Budget | 36 pages / 7 learning hours |
...
| Terminology block | provisional, pending [Decision 0019](../../decisions/0019-chapter12-optimization-terminology-and-boundary.md) |
...
**One of the four sampled, and the reason is the rubric.** This is where the book's assessment instrument changes from scored to unscored, and no decision record proposes the change. A pilot here tests the unscored instrument for the first time; if it does not work, six chapters are affected. See [Decision 0025](../../decisions/0025-validation-architecture.md) clause 4.
...
6. **Chapter audit.** The 36-page / 7-hour architecture holds unless evidence justifies reopening it; all 8 sections still serve their governed jobs; central question and core competence intact; reveal order coherent; citation keys resolve and source-note cautions are respected; no exercise exposes an answer, rubric, or parallel form before production.
7. **Freeze decision.** `FREEZE` · `FREEZE WITH DEFERRED NON-BLOCKING ITEMS` · `REVISE AND RE-PILOT` · `REOPEN GOVERNED DECISION`.

**Gate 7 additionally requires that [Decision 0019](../../decisions/0019-chapter12-optimization-terminology-and-boundary.md) has been adjudicated.** This chapter's terminology and scope are provisional until it is, and a chapter cannot freeze on a provisional boundary.
