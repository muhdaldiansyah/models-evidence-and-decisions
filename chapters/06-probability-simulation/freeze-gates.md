...
| | |
|---|---|
| Chapter | **Probability, Prediction, and Simulation** |
| Manuscript | complete — 8 sections, 11,975 words |
| Budget | 34 pages / 7 learning hours |
| Self-explanation pauses | 3 |
| Planted-defect diagnosis task | yes — 5 defects |
| Parallel cold-transfer forms | A and B, 45 minutes |
| Rubric mode | **S — scored dimensions (0/1/2)** |
| Terminology block | provisional, pending [Decision 0013](../../decisions/0013-chapter6-probability-terminology-and-notation.md) |
...
**The book's first announced notation exception**, and the first chapter that asks a reader to write a probability down before teaching anything. Both are pilot-visible. The opening task in particular is the earliest place a reader can decide the book is asking too much, and a refusal to attempt it is a finding rather than a failure.
...
6. **Chapter audit.** The 34-page / 7-hour architecture holds unless evidence justifies reopening it; all 8 sections still serve their governed jobs; central question and core competence intact; reveal order coherent; citation keys resolve and source-note cautions are respected; no exercise exposes an answer, rubric, or parallel form before production.
7. **Freeze decision.** `FREEZE` · `FREEZE WITH DEFERRED NON-BLOCKING ITEMS` · `REVISE AND RE-PILOT` · `REOPEN GOVERNED DECISION`.

**Gate 7 additionally requires that [Decision 0013](../../decisions/0013-chapter6-probability-terminology-and-notation.md) has been adjudicated.** This chapter's terminology and scope are provisional until it is, and a chapter cannot freeze on a provisional boundary.
