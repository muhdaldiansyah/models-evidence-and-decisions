...
- **The first two book-level checks have been run, and they are mechanical ones.** With all seventeen specifications now written, the synchronization `CLAUDE.md` requires between this file and each `spec.md` was checked across all **fifty-one governed fields** — seventeen titles, seventeen central questions, seventeen core competences. **Sixteen chapters match exactly.** The one divergence is Chapter 1's core competence, described in the bullet below. Separately, every forward routing in the book — **152 of them**, drawn from the specifications' *deferred to later chapters* lists and the decision records' *what Chapter N does not do* lists — was checked against the chapter it points at. **All but two are discharged**, either by delivery or by an explicit refusal on the page. Neither check is a substitute for reading; both are the kind that could not be run until the draft was complete.
- **Two topics are routed forward to chapters that never mention them.** [Decision 0017](decisions/0017-chapter10-values-terminology-and-boundary.md) clause 9 routes **preference elicitation** to Chapter 11; `chapters/11-decisions-voi/spec.md`, that chapter's manuscript, and [Decision 0018](decisions/0018-chapter11-decision-terminology-and-boundary.md) do not mention it, to teach it or to decline it. `chapters/10-values-alternatives/spec.md` sends the same topic to the depth curriculum instead, so a lower-authority file contradicts a higher one — the **second** such disagreement about a destination, after the trade-offs one recorded above. And `chapters/04-observation-provenance/spec.md` routes **weighting, post-stratification, imputation, deletion rules, and selection models** to Chapter 8, whose scope boundary accepts none of them; Chapter 8 names **measurement-error correction** as missing and supplies nothing about the other five. In both cases the material has fallen between two chapters rather than being placed in the depth curriculum on purpose. **Repairing either changes a chapter's scope, so both are surfaced rather than fixed.**
- **The validation apparatus now exists for every chapter, and it establishes that a full pilot is undesignable as specified.** Before [Decision 0025](decisions/0025-validation-architecture.md) it existed for Chapter 1 and for no other chapter: a willing reviewer or reader arriving today could have worked on Chapter 1 and nothing else. [`validation/`](validation/) now holds the gate status, one pilot protocol, one capture template, and two SME packets, and every chapter carries a `freeze-gates.md`. The record also computes what a whole-book pilot costs — **100 learning hours across 503 pages plus 26.2 hours of transfer production, about 126 hours per reader** — and concludes that the pilot must **sample four chapters** (1, 8, 12, 16) rather than seventeen. **All 119 gates in the book are open or not sampled; none is closed, and no reviewer or reader has been approached for any of them.**
- **The book uses two different assessment instruments and nothing records the change.** Chapters 1–11 supply a cold-transfer rubric scored 0–2 by dimension; **Chapters 12–17 supply a review instrument that states it scores and records nothing.** The change happened in the artifacts, not in the governance — no decision proposes it, and `chapters/12-optimization-robustness/spec.md` still carries a seven-item "Rubric dimensions" section as though its rubric scored them. The unscored direction has a real argument behind it at [Decision 0023](decisions/0023-chapter16-integration-terminology-and-boundary.md) clause 5, but that argument was about one preserved artefact rather than about rubrics generally. [Decision 0025](decisions/0025-validation-architecture.md) clause 4 surfaces it and does not choose.
- **A miscount ran from Chapter 13 to the last page of the book.** Chapters 3–12 number the water anchor's recurrences by chapter number; Chapter 13 restarted at *tenth*, so Chapters 10 and 13 both claimed the tenth recurrence, 11 and 14 the eleventh, and 12 and 15 the twelfth. Chapter 16's case is not the anchor, so the anchor appears in sixteen chapters and Chapter 17 is its **sixteenth** appearance, not its thirteenth — a sentence that had reached the final chapter's manuscript. Corrected across four chapter directories and four decision records, along with the Gate 1 depth counter that carried the same error. **The publication-gate convention that records these counts had also lapsed after Chapter 12 and is restored in all five remaining chapters.**
- **The anchor changes size between Chapter 9 and Chapter 13, and nothing says so.** Chapters 1–9 operate a utility with **14.0 ML** of usable storage and **8.4 ML/day** of treated-water input. Chapters 13 and 14 operate one with **260 ML** of capacity and **100 ML/day** of standing production — **18.6 times the storage and 11.9 times the production** — and `chapters/13-dynamics-feedback/chapter.md` identifies the two explicitly: the stock is one "you have known since Chapter 1 — it is the reading Chapter 1 discovered might be wrong." Chapters 10, 11, and 12 state no volumes, so nothing bridges the jump. The same passage carries a second conflict: Chapter 13's **two-day** observation delay and **two-day** production delay are attributed to Chapter 1's screen, and Chapter 1 stipulates a **six-hour** production ramp-up and a **25-minute** verification. Chapter 1's screen establishes the two *kinds* of delay, not those magnitudes. **Both are frozen case facts in files whose manuscripts may not contradict them, and repairing either changes arithmetic across several chapters, so both are surfaced rather than fixed.**
- **The four remaining book-level audits have now been run, and the source notes needed repair.** Backward references: 147 numbers attributed to earlier chapters were checked, and one mis-attribution was found and corrected — Chapter 15 credited Chapter 3 with establishing that "some of the utility's fifteen points were badly sited in 1994", where Chapter 3 examined one monitoring point and gave no date. Canon first use: **161 registered terms, no violations** — the only two terms appearing before their chapter are explicit "reserved for Chapter N" warnings. Citation locators: every page cited in every manuscript was checked against what its source note records as read, and **two notes were stale**. `sources/deaton2016rct.md` still asserted that "nothing after printed p. 11 has been read" while Chapter 9 quotes pp. 27–29, a reading [Decision 0016](decisions/0016-chapter9-synthesis-terminology-and-boundary.md) records; `sources/pearl2009causal.md` recorded pp. 96–101 and 119–120 while Chapter 7 cites pp. 109, 114, 117, 121, and 122, including the page the whole chapter is organised on. Both notes now record what was read. **All 29 sources cited with page locators are clean.**
- **The book can now be read as a book, and the reader is now told how to work it.** [Decision 0026](decisions/0026-front-matter-and-build.md) adds [`front-matter/`](front-matter/) — a [preface](front-matter/preface.md), a [reader's guide](front-matter/how-to-use-this-book.md), and a [term index](front-matter/term-index.md) generated from the registry — and a `Makefile` that builds HTML, EPUB, and DOCX from the seventeen chapters in filesystem order. **The reader's guide carries the part that was actually missing:** every chapter conceals a rubric, a feedback file, and an unassigned transfer form, and nothing had ever told a reader not to open them. The preface states on its first page what Chapter 17 §8 states on the last — no pilot data, no SME review, no transfer claim. **`make check` codifies the verification suite** that had been run by hand at every chapter boundary, holds the Chapter 1 core-competence divergence as a tracked exception rather than a permanent failure, and fails on any new divergence. **PDF is not built** — no LaTeX engine is installed and no typesetting decision has been made.
...
- Reader-facing front matter and the build are governed by [Decision 0026](decisions/0026-front-matter-and-build.md). **`make check` runs the repository verification suite**, and every build format depends on it, so a repository that fails verification cannot be built.
- Validation is governed by [Decision 0025](decisions/0025-validation-architecture.md) and lives in [`validation/`](validation/). **Gate status is true in [`validation/gate-status.md`](validation/gate-status.md) and nowhere else**; a chapter's `freeze-gates.md` describes its gates without asserting whether one is closed.
